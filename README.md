...
есть **`Самопроверка.ps1`**: 50 проверок, которые прогоняются одной кнопкой и
...
```
Пройдено 50 из 50 — всё в порядке.
```
...
| `Самопроверка.ps1` | 50 проверок кода, шифрования и вёрстки |
...
- Программа больше не закрывается сама. Причина была настоящая: в PowerShell
  внутри `.GetNewClosure()` обращение к `$script:`-переменной даёт `$null`, а
  запись пропадает. Из-за этого «Сохранить» в настройках роняло окно, и ещё
  три места молча не работали. Теперь есть перехватчик исключений: ошибка
  пишется в журнал, окно остаётся открытым.
- **Запускается больше четырёх аккаунтов.** Жёсткого предела не было — при
  запросе токена Roblox отвечал «слишком часто», ошибка вылетала мимо цикла
  повторов, и аккаунт молча пропадал из очереди. Теперь пауза соблюдается,
  аккаунт возвращается в очередь, а в конце показывается честный отчёт: кто
  не запустился и почему.
- **Открытые клиенты не теряются.** Закрыл менеджер, оставив Roblox
  запущенным, открыл снова — окна опознаются по заголовку и по номеру
  процесса. Раньше они числились закрытыми, и «Запустить» открывало дубликат.
- **Исправлена графика при массовом запуске** — общий файл настроек Roblox
  успевал перезаписаться раньше, чем предыдущий клиент его дочитывал.
  Подробности в разделе про настройки графики ниже.

**Настройка под себя.**

- **Мастер первого запуска** — пять шагов: железо, тема, аккаунты, кто
  основной, готово. Любой шаг пропускается. Открывается заново из «Справки».
- **Готовые наборы настроек аккаунта** — «Основной, красиво»,
  «Сбалансированно», «Твинк, легко тянет», «Фоновый фарм». Один щелчок в
  окне аккаунта или по правому клику на карточке.
- **Подбор под железо** — AltHub смотрит память и ядра и говорит, сколько
  клиентов машина вытянет. Если просишь больше — предупредит, но не запретит.
- **Четыре новых способа добавить аккаунт** — пачкой, из браузера, файлом
  и по ссылке-приглашению.
...
- **Конструктор своих тем** — простой и подробный режим, живое превью, обмен
  темами через файл. Плюс 10 встроенных тем вместо трёх.
- **Темы стали различаться.** «Небо» и «Светлая» были одинаковыми не на
  глаз, а буквально: у всех светлых тем панели выходили чисто белыми из-за
  ошибки в формуле. Исправлено, различимость проверяется автоматически.
- Все окна переведены на **раскладчик**: в коде не осталось ни одной
  координаты, вписанной числом. Размеры считаются от реальной ширины текста,
  поэтому при масштабе экрана 125% и 150% ничего не наезжает и не обрезается.
  Вёрстка проверяется автоматически на всех окнах и всех трёх масштабах.
- **Настройки разложены по смыслу** — шесть разделов вместо одной простыни
  из одиннадцати галочек. Рядом с каждой настройкой написано, что она делает
  и что советуем. Внизу — «Вернуть рекомендованное».
- Единый низ у всех диалогов: главное действие справа, «Отмена» рядом с ним.

**Нагрузка на систему.**

- Аватарки грузятся не блокируя окно. Раньше при недоступной сети окно могло
  замереть до двух с половиной минут за один такт обновления.
- Поиск не пересобирает карточки на каждую букву, картинки и меню
  освобождаются, статусные точки перерисовываются только когда изменились.
- В покое с двадцатью аккаунтами такт обновления занимает около 10 мс.

**Внутри.**

- `AltHub.ps1` разбит по смыслу на модули: было 6700 строк в одном файле,
  стало 580 в точке входа и отдельные файлы под данные, действия и внешний
  вид. Так проще прочитать и убедиться, что лишнего нет.
- Самопроверка выросла с 36 до 50 проверок.
