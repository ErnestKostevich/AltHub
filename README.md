...
<p align="center">
  <img alt="тестовая версия 1.4.1-rc1" src="https://img.shields.io/badge/версия-1.4.1--rc1-8b6cff">
...
![AltHub 1.4 — Dashboard](docs/menu-dashboard.png)

> **Тестовая сборка 1.4.1-rc1.** Проверяем создание файла настроек Roblox
> на компьютере, где клиент сам не создаёт `GlobalBasicSettings_13.xml`.
> Работу в игре нужно подтвердить на этом компьютере до выпуска стабильной версии.

Три интерфейса входят в один архив и переключаются в **Настройки → Вид**:

| Pulse | H₂O Original |
|---|---|
| ![Pulse](docs/menu-pulse.png) | ![H₂O Original](docs/menu-water.png) |
...
Семь разделов вместо одной простыни. Рядом с каждой настройкой написано,
...
1. Двойной клик по **`AltHub.vbs`** — запускается без чёрного окна консоли.
   (Или `Запустить AltHub.cmd`, если `.vbs` отключён политикой.) Ярлык на рабочий
...
**Если клавиша не сработала.** F10 — обычная клавиша, и её может держать что-то
другое: игра, оверлей, второй менеджер. Windows в таком случае просто не отдаёт
её AltHub. Раньше на этом всё и заканчивалось молча, и выглядело это как «ничего
не работает». Теперь программа говорит об этом прямо и сама переезжает на первую
свободную из списка `F8, F9, F10, F11, Ctrl+Alt+A`. Клавишу можно выбрать
руками в настройках, раздел «Из браузера» — там же написано текущее состояние:
на какой клавише ждём и отзывалось ли расширение хоть раз.

А можно вообще не пользоваться клавишей: значок AltHub на панели браузера
...
побывал и был убран. Он существует ровно для одного — прятать
`navigator.webdriver` от проверок на бота.

И сразу честно о том, чего я НЕ проверил: опыт «запустить браузер с флагом и
без и посмотреть, что видит страница» я начал и не довёл. Поэтому утверждать,
что флаг ничего не менял, не буду — это было бы предположение, выданное за
факт. Убран он по другой причине, которая от опыта не зависит: строка,
выглядящая как обход защиты от ботов, в этом проекте не нужна независимо от
того, работает она или нет.

Что проверяется автоматически: в коде нет ни этого флага, ни `excludeSwitches`,
ни `--headless`, ни подмены `navigator.webdriver`. Отдельная проверка в
самопроверке падает, если такое появится.
...
Архив разложен так, чтобы с первого взгляда было ясно, что нажимать.

```
AltHub-1.4.1-rc1\
  Запустить AltHub.cmd        ← единственное, что нужно
  Ярлык на рабочий стол.cmd
  ПРОЧТИ МЕНЯ.txt
  README.md, LICENSE
  AltHub.ps1, AltHub.vbs      ← сама программа
  modules\   extension\   tools\   docs\
  Проверки\                   ← самопроверка и диагностика
```

| Файл | Что делает |
|---|---|
| `Запустить AltHub.cmd` | Запуск. Сам выбирает путь без чёрного окна консоли |
| `Ярлык на рабочий стол.cmd` | Создаёт ярлык «AltHub» со своей иконкой |
| `AltHub.vbs` | Запуск без окна консоли — на него указывает ярлык |
| `AltHub.ps1` | Точка входа: состояние, журнал, подключение модулей |
| `tools\shortcut.ps1` | Собственно создание ярлыка |
| `modules\Storage.ps1` | Шифрование и хранение аккаунтов на диске, резервные копии |
...
| `modules\WindowTools.ps1` | Заголовки окон и раскладка |
...
| `modules\Theme.ps1` | Цвета, шрифты, нарисованные значки и элементы |
| `modules\Layout.ps1` | Раскладчик диалоговых окон |
| `modules\MenuStyles.ps1` | Выбор вида меню и колонки карточек |
| `modules\menus\` | Описания трёх видов меню: Pulse, H₂O Original, Dashboard |
| `modules\UiModern.ps1` | Вид меню Dashboard: вкладки сверху, плитки аккаунтов сеткой |
| `modules\UiPulse.ps1` | Вид меню Pulse: боковое меню и список строками, как в таблице |
...
| `Проверки\Самопроверка.ps1` | Проверки кода, шифрования и вёрстки |
| `Проверки\Проверка моста.ps1` | Поднимает приём и стучится в него так же, как расширение |
| `Проверки\Проверка запуска.ps1` | Запускает программу как человек и смотрит, что видно на экране |
| `Проверки\Запустить с окном.cmd` | Запуск с видимой консолью — чтобы увидеть ошибку, если что-то не так |
| `Проверки\Проверка входов.ps1` | Проверяет, продлевает ли Roblox вход |
| `Проверки\Диагностика.ps1` | Показывает, что отвечает Roblox, если запуск не идёт |
| `data\accounts.dat` | Твои аккаунты, **зашифрованы** (создаётся сам) |
| `data\backups\accounts\` | Пять последних копий `accounts.dat`, тоже зашифрованы |
...
**Папку `data\` никому не пересылай.** Там твои входы: кто получит эту папку
вместе с учётной записью Windows, войдёт в твои аккаунты без пароля.
...
переработал раскладку окна, нарисовал значки и нашёл, почему окно входа
перезапускалось посреди ввода пароля. Спасибо.

Перед выпуском код разобрали двадцать независимых проверок — по файлу на
каждую и отдельно по сторонним источникам о Roblox, капче и Chrome. Ниже —
что из найденного исправлено.

### Три вида меню на выбор

**Настройки → Вид.** Меняется сразу, без перезапуска:
окно перестраивается, запущенные Roblox и очередь запуска не прерываются.
Цветовая тема выбирается отдельно и тоже применяется сразу.

- **Dashboard** (у новых людей по умолчанию) — вкладки разделов сверху, плитки
  аккаунтов встают в сетку: сколько помещается по ширине окна, столько и в ряд.
  На плитке — аватарка, метка состояния напротив имени, игра, одна строка
  настроек и три кнопки словами: «Запустить», «Изменить», «Закрыть». Поиск стоит
  рядом с аккаунтами, шапка — одной строкой. Игры, профили и статистика — тоже
  плитками.
- **H₂O Original** — авторский вид H₂O: действия в боковом меню, значки-рисунки, «+» прямо на
  кнопках разделов, состояние аккаунта рамкой аватарки. Значки есть только здесь.
  В обычном окне список одноколоночный; в полном экране карточки становятся
  в две колонки, а не растягиваются через весь монитор.
- **Pulse** — рабочий список строками, как в таблице: имя и ник, игра и
  настройки, три кнопки одной ширины у правого края. Колонки делят ширину окна
  по долям. Состояние — цветом рамки аватарки, сводка — в боковом меню. Глифы
  H₂O здесь не используются.

#### Dashboard

![Dashboard](docs/menu-dashboard.png)

#### H₂O Original

![H₂O Original](docs/menu-water.png)

#### Pulse

![Pulse](docs/menu-pulse.png)

Размер и положение запоминаются отдельно для каждого вида. Развёрнутый H₂O
не растягивает Dashboard или Pulse при следующем переключении.
После обновления с ранней сборки 1.4 старые полноэкранные размеры сбрасываются
один раз, потому что они были рассчитаны под прежние раздутые панели. Дальше
размеры снова запоминаются обычно.

**Рассчитано по ширине окна и по масштабу экрана.** Перед выпуском на экране с
масштабом 200% «Мои игры», «Профили» и «Статистика» были кривыми: заголовки
резались пополам, часть подписей не рисовалась вовсе. Причина — высоты,
вписанные числами под 100%. Теперь высота любой подписи — это высота строки
её шрифта, ширина кнопки — ширина её надписи, всё нижнее стоит от
фактического низа верхнего. То же сделано со всеми диалогами: сообщения, ввод,
аккаунт, мастер добавления, конструктор тем, популярные игры. Самопроверка
собирает каждое окно в каждом виде меню на 100%, 125%, 150% и 200% и
проверяет, что ни одна подпись не ниже строки шрифта, ничего не вылезает и не
налезает. Полосы прокрутки на тёмной теме тоже тёмные. Размер и место окна
запоминаются.

### Аккаунты больше не теряются

- Если `accounts.dat` не читается (переустановили Windows, перенесли папку),
  программа **закрывается, не трогая файл**. Раньше она писала «файл не
  тронут» — и при закрытии перезаписывала его пустым списком.
- Перед каждой записью файл проверяется пробным чтением, пять последних
  копий лежат в `data\backups\accounts`.
- «Метка» с пустым полем больше не стирает метки у всех отмеченных.
- Отметки у аккаунтов, скрытых поиском, больше не пропадают.

### Живые входы больше не объявляются мёртвыми

Мёртвым вход считается **только** когда Roblox прямо ответил «кука
недействительна». Нет интернета, Roblox попросил сбавить темп, сервер не
ответил — состояние карточки не меняется, а программа так и говорит.
«Проверить входы» идёт фоновой очередью с паузами — окно не замирает, и
частые запросы больше не выбивают Roblox на «слишком часто».

### Вход из браузера и окно входа

- Приём входа принимает запросы только от расширения AltHub с одноразовым
  ключом. Раньше любая открытая страница могла постучаться в этот порт.
- Если Windows не даёт занять порт по правам, так и написано — а не «порт
  занят».
- Окно входа перезапускается, **только** если страница правда не загрузилась
  за 20 секунд, и не больше одного раза. Живую страницу, где ты печатаешь
  пароль, оно не трогает.
- Недобитые процессы браузера от прошлых входов гасятся перед каждым новым —
  больше не зависает после нескольких входов подряд.
- Профиль окна входа не чистится и переносится при обновлении: Roblox со
  временем узнаёт устройство, и капча становится короче.

### Настройки графики

Раньше программа писала в журнал «графика применена», даже если Roblox
переименовал поле и ничего не записалось. Теперь несработавшая запись — это
ошибка со словами. Резервная копия настроек Roblox больше не откатывает твои
более свежие ручные правки.

### Мелочи, которые молчали

«Окна» правым кликом, «Закрыть» без запущенных, ■ на незапущенном
аккаунте, «Скопировать журнал» при занятом буфере — теперь отвечают. «Закрыть»
без отметок больше не закрывает все окна молча. Цифры 1–5 открывают разделы по
порядку меню. Запуск по расписанию не повторяется после перезапуска программы
и ночью не ждёт человека в модальном окне.

### Мультизапуск — честно

Первый запуск нескольких окон сразу один раз показывает, чем это грозит, —
см. [ниже](#честно-о-рисках). Если после обновления Roblox каждое новое окно
начнёт закрывать предыдущее, AltHub это заметит и скажет прямо.
...
Сверху в этом окне Chrome for Testing показывает свою жёлтую полоску «версия
предназначена только для тестирования». Мы её не прячем. Флаг, который её
убирает (`--test-type`), заодно глушит и остальные предупреждения браузера, а
прятать от человека то, что говорит ему сам браузер, — не наше дело. Полоска
ни на что не влияет, входу не мешает.
...
человек в этот момент играл. В 1.4 поведение стало строже: каждый новый клиент
сразу запускается свёрнутым и повторно сворачивается в момент появления
настоящего окна Roblox. Автораскладка меняет его будущую позицию, но не
разворачивает. Нужное окно можно явно показать из карточки аккаунта.

Когда окно Roblox закрывают из AltHub, фокус возвращается программе, которая
была активна до менеджера: браузеру, Discord или другому окну Roblox. Если
клиент закрылся сам, AltHub только обновляет карточку и журнал — своё окно
поверх остальных не показывает.
...
Теперь рядом лежит **`Проверки\Проверка запуска.ps1`**: он запускает программу так же,
...
есть **`Проверки\Проверка входов.ps1`**: он обходит обычные запросы на твоём же аккаунте
...
Прогони `Проверки\Проверка входов.ps1` сам, если не веришь — запросы только на чтение,
...
Roblox хранит настройки клиента в одном общем файле. AltHub ищет точное имя
файла во всём каталоге обычной установки и внутри пакета Roblox из Microsoft
Store, включая `LocalState`:
...
У некоторых пользователей Roblox запускается, но вообще не создаёт этот XML.
В таком случае AltHub создаёт минимальный файл `UserGameSettings`, записывает
в него настройки аккаунта и затем запускает клиент. Существующий файл при
этом не пересоздаётся. Если создать XML нельзя, Roblox всё равно открывается,
а причина пропуска настроек появляется в журнале. Запись XML проверяется до
старта; окончательное применение значений зависит от поведения клиента Roblox.
...
Теперь AltHub перед записью настроек следующего аккаунта **дожидается окна и
не менее 15 секунд от старта предыдущего клиента**. Перед заменой файла новый
XML повторно читается и сверяется со всеми выбранными значениями. Ждём только
когда настройки соседних аккаунтов реально отличаются: пять одинаковых
твинков запускаются с прежней скоростью.

Режим «в окне» дополнительно снимает maximized у появившегося окна перед
сворачиванием. Сам процесс больше не запускается через `WindowStyle=Minimized`:
у некоторых сборок Roblox это портило восстановленный размер и окно потом
раскрывалось на весь экран вопреки `Fullscreen=false`.
...
| `F5` | Обновить список |
| `1` … `5` | Разделы слева, по порядку меню |
| `Esc` | Сбросить поиск |
...
Иметь несколько аккаунтов Roblox разрешает сам: переключатель до пяти
аккаунтов есть прямо в клиенте. AltHub не меняет Roblox, не читает его память и
не даёт преимущества в играх — он хранит входы, запускает официальный клиент и
раскладывает окна.

А вот несколько клиентов на одном компьютере Roblox официально называет
**неподдерживаемым**: это может перестать работать после любого обновления, и
AltHub этого не контролирует. Уже известный побочный эффект — **переход между
играми (телепорт) часто срабатывает только в последнем запущенном окне.**

Банят за поступки, а не за программу: за ботов, накрутку, обход наказаний. При
этом правила Roblox позволяют наказать все аккаунты, которые он сочтёт твоими, а
отдельные игры своими правилами запрещают альты. Подтверждённых банов именно за
менеджер аккаунтов мы не знаем, но и письменного разрешения нет — решать тебе.
...
**Ничего не открылось после `Запустить AltHub.cmd`**
...
Запусти `Проверки\Диагностика.ps1` — она покажет точные коды ответов. Куки в её выводе
...
Проверить вручную, какая версия берётся, можно `Проверки\Самопроверка.ps1` —
...
- пусть первым делом запустят `Проверки\Самопроверка.ps1`: она сама покажет, что в коде
...
Правый клик по `Проверки\Самопроверка.ps1` → «Выполнить с помощью PowerShell».
...
Прогонит 90 проверок: синтаксис всех файлов, оба режима шифрования (включая
...
Ожидаемый результат — «Пройдено 90 из 90».
