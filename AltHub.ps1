...
function Write-RamLog {
    ...
    # Вход по паролю из программы убран (Roblox закрыл его проверкой
    # proofofwork), но вырезку оставляем: мало ли что попадёт в текст ошибки.
...
function Start-AltHub {
    ...
    # Одна копия на компьютер. Если программа уже работает — поднимаем её окно
    # и уходим. Это главная страховка от «программа пропала»: что бы ни
    # случилось с окном, повторный клик по ярлыку всегда его возвращает.
    if (Test-RamAlreadyRunning) { return }
    ...
    # Значки расширения рисуем при КАЖДОМ запуске, а не только когда человек
    # откроет инструкцию. Иначе он может пойти ставить расширение раньше — и
    # Chrome будет ругаться на отсутствующие файлы значков, а кнопки на панели
    # не появится. Рисование одноразовое: если файлы уже есть, ничего не делает.
    Invoke-RamSafe -What 'значки расширения' -Body { [void](Save-RamExtensionIcons) }
    ...
    # Application::Run, а НЕ ShowDialog.
    #
    # ShowDialog завершается не только когда окно закрыли, но и когда его
    # ПРЯЧУТ: как только Visible становится false, модальный цикл выходит.
    # Из-за этого «крестик убирает в часы» убивал программу — окно пряталось,
    # ShowDialog возвращал управление, скрипт доходил до конца и процесс
