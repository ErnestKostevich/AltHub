...
<p align="center">
  <img alt="версия 1.2.1" src="https://img.shields.io/badge/версия-1.2.1-8b6cff">
...
есть **`Самопроверка.ps1`**: 59 проверок, которые прогоняются одной кнопкой и
...
```
Пройдено 59 из 59 — всё в порядке.
```
...
## Шесть способов добавить аккаунт
...
| **По логину и паролю** | Привычно тем, кто пришёл из других менеджеров. Ограничения — ниже. |
...
### Про вход по логину и паролю — честно

Пароль уходит ровно в один адрес: `auth.roblox.com/v2/login`, то есть на
официальный сервер Roblox, тем же запросом, что делает обычная страница входа.
**Он нигде не сохраняется** — ни в аккаунте, ни в настройках, ни в журнале:
живёт только в памяти на время запроса. Обратно берётся та же `.ROBLOSECURITY`,
что и при любом другом способе. Двухфакторка поддерживается — код вводишь ты.

**Капчу за тебя никто не разгадывает.** Roblox просит её часто; тогда AltHub
открывает настоящую страницу входа, и капчу проходишь ты сам. Ни своих
решателей, ни платных сервисов разгадывания в коде нет — это проверяется
автоматически в `Самопроверка.ps1`. Именно за такие вещи программы этого рода
и попадают в чёрные списки антивирусов.

Поэтому способ и называется дополнительным: самый надёжный — «Из приложения
Roblox», там пароль не нужен вовсе.
...
| `Самопроверка.ps1` | 59 проверок кода, шифрования и вёрстки |
...
### 1.2.1 — окно могло не появиться

`AltHub.vbs` из 1.2 запускал PowerShell с режимом окна `0`. Выглядит логично —
«не показывать консоль», — но этот режим попадает в `STARTUPINFO` процесса как
`SW_HIDE`, а Windows применяет его к **первому окну программы**. AltHub при этом
работал, но его нигде не было видно: со стороны выглядело как «не запускается,
сразу закрывается».

Исправлено в двух местах: `.vbs` больше не прячет окно процесса (консоль прячет
сам PowerShell), а программа при запуске проверяет через Win32, видно ли её
окно, и показывает его принудительно, каким бы способом её ни запустили.

Заодно: номера аккаунтов Roblox больше двух миллиардов (`4062608487`) не
влезали в `Int32` и роняли такт обновления — везде заменено на `Int64`.
И добавлен **`Запустить с окном.cmd`**: если что-то не открывается, он покажет
точную ошибку.

Версия по первому отзыву со стороны — от человека, привыкшего к другому
менеджеру аккаунтов. Про то, обо что спотыкаются в первые пять минут.

**Окно больше не пропадает.**

- Свернул — и приложения нигде нет. Так и было: при сворачивании в часы окно
  убиралось в том числе из панели задач, а Windows 11 прячет новый значок под
  стрелку `^`. Человек оставался без окна и без значка. Теперь окно прячется
  **только если значок действительно создался**, а при первом сворачивании
  всплывает подсказка, где его искать.
- **Куда девается окно — теперь настройка, а не догадка.** Отдельно для кнопки
  свернуть и отдельно для крестика: «свернуть в панель задач» или «убрать
  в часы». По умолчанию оба ведут себя обычно — программа не должна исчезать
  у того, кто ничего не настраивал.
- Значок в часах возвращает окно по одиночному клику, а не только по двойному.
...
- **Менеджер сам замечает**, что в приложении Roblox сменился аккаунт, и
  ...
  поверх игры.
- **Вход по логину и паролю** — дополнительный способ, в мастере под кнопкой
  «Ещё способы». Пароль уходит только на сервер Roblox и **нигде не
  сохраняется**: ни в аккаунте, ни в настройках, ни в журнале. Двухфакторка
  поддерживается — код вводишь ты сам.
  Если Roblox просит капчу, AltHub открывает настоящую страницу входа, и
  капчу проходишь ты. **За тебя её никто не разгадывает** — ни своими силами,
  ни через сервисы: именно за это программы такого рода и попадают в чёрные
  списки. Самый надёжный способ всё равно «Из приложения Roblox», там пароль
  не нужен вовсе.
