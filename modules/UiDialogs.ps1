...
function Show-RamSettingsDialog {
    ...
    $cbTray    = & $addCheck $pm 'Сворачивать в часы, а не в панель задач' ([bool]$s.MinimizeToTray) `
                 'Значок уходит в область уведомлений рядом с часами и не занимает место на панели.'
    ...
    $btnReset = New-RamButton -Text 'Вернуть рекомендованное' -Height $m.RowH -Kind 'ghost' `
        ...
        $cbTray.Tag.Checked    = [bool]$def.MinimizeToTray
        $cbLogFile.Tag.Checked = [bool]$def.LogToFile
        foreach ($c in @($cbRename,$cbRestart,$cbCheck,$cbTile,$cbSavedW,$cbHotkey,$cbCompact,$cbEmoji,$cbConfirm,$cbTray,$cbLogFile)) {
            $c.Invalidate()
        }
